...
# include skips? it's another 3000. 
 ...
 coefs_labels = coefs_labels[(overlap | postyear | unchanging),]
 #df3coefs_labels = coefs_labels[(overlap | postyear | unchanging),]
...
#### Seems like one hot encoding maybe isn't best practice after all
...
clustrang13 = randomForest(tcCluster~., data =clusttrain[,c("tcCluster",featureNames),with=F], 
										 importance=T,
                     keep.forest=T,
                     mtry = floor(sqrt(length(featureNames)))*2)
