...
#	loader = function(x){
#		indt = get(load(x))
#		outdt = indt[yr_start <= mapyear & yr_end >= mapyear,]
#		rm(indt)
#		return(outdt)
#	}

	rfout = mclapply(rffiles, function(x)get(load(x)), mc.cores=detectCores())
	#rfout = mclapply(rffiles, loader, mc.cores=detectCores())
# don't forget shade classification!!!!
# do all years
	rfdt_pre = rbindlist(rfout)
...
# pick by year
	rfdt = rfdt_pre[yr_start <= mapyear & yr_end >= mapyear,]
